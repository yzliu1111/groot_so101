...
def build_oxe_observation(
    ...
    joint_6 = policy_obs["joint_pos"].detach().cpu().numpy()[0].astype(np.float32)

    # GR00T OXE/DROID 的 joint_position 是 7D。这里把 SO101 的 6D 放前 6 维，
    # 第 7 维补 0。这是一个很粗的兼容层，也是 joint 控制路线效果差的主要风险。
    joint_7 = np.zeros((1, 1, 7), dtype=np.float32)
    joint_7[0, 0, :6] = joint_6

    # gripper_position 单独作为 state key，shape 必须是 `(B, T, D)`。
    gripper = np.array([[[joint_6[5]]]], dtype=np.float32)
...
def joint_action_to_leisaac_tensor(
    action: dict[str, np.ndarray], env_device: str, fallback_joint: torch.Tensor
) -> torch.Tensor:
    ...
    输出：
    - shape `(1,6)` 的 torch tensor，可直接传给 `env.step()`。
...
def eef_action_to_leisaac_tensor(
    ...
    if "eef_9d" not in action:
        ...
        gripper = policy_obs["joint_pos"].detach().cpu().numpy()[0, 5:6].astype(np.float32)
    ...
    if "gripper_position" in action:
        ...
        gripper = float(policy_obs["joint_pos"].detach().cpu().numpy()[0, 5])
...
def smart_task_metrics(env: Any) -> dict[str, float]:
    ...
    base_idx = robot.data.body_names.index("base")
...
def print_metrics(env: Any, prefix: str) -> None:
    """用统一格式打印 SmartTask 诊断指标。"""

    metrics = smart_task_metrics(env)
    print(
        "[runner] "
        f"{prefix} metrics "
...
def parse_args() -> argparse.Namespace:
    ...
    # IsaacLab task id。LeIsaac 在 import leisaac 时注册这个 gymnasium env。
    parser.add_argument("--task", default="LeIsaac-SO101-SmartTask-v0")
...
def main() -> None:
    ...
    try:
        ...
        print(f"[runner] control mode: {args.control_mode} teleop_device={teleop_device}", flush=True)
        ...
        print_metrics(env, "initial")

        # warmup：重复把当前 observation 放入 history，确保 history 至少有两帧。
        for _ in range(args.warmup_frames):
            build_oxe_observation(policy_obs, history, args.instruction)
        ...
        for call_idx in range(args.max_policy_calls):
            ...
            for step_idx in range(num_action_steps):
                if args.control_mode == "eef":
                    ...
                    command = eef_action_to_leisaac_tensor(step_action, env.device, policy_obs)
                else:
                    ...
                    if "joint_position" in action and action["joint_position"].shape[1] > step_idx:
                        ...
                        step_action = action
                    command = joint_action_to_leisaac_tensor(step_action, env.device, policy_obs["joint_pos"])
                    ...
                    if args.control_mode == "eef":
                        ...
                        cur_np = np.concatenate(
                            [
                                ...
                                policy_obs["joint_pos"].detach().cpu().numpy()[:, 5:6],
                            ],
                            axis=1,
                        )
                    else:
            ...
            print_metrics(env, f"after policy call {call_idx + 1}")
